...
/* ==================== GNU-EFI HEADERS (always include) ==================== */
#include <efi.h>
#include <efilib.h>
#include <efi/efipciio.h>
/* ==================== DISTRO-SPECIFIC OVERRIDES (via -D flags) ==================== */
/* Примеры использования:
 *   -D USE_LEGACY_PCIIO_PATH     # если в твоём gnu-efi заголовки в protocol/
 *   -D GNUEFI_VERSION=0x040000   # если нужно включить фоллбэки под старую версию
 */

/* Если вдруг в дистрибутиве заголовки лежат в protocol/ вместо efi/ */
#ifdef USE_LEGACY_PCIIO_PATH
  #undef include_efipciio_done
  #ifndef include_efipciio_done
    #include <protocol/pciio.h>
    #define include_efipciio_done 1
  #endif
#endif

/* GUID ReadyToBoot: в некоторых сборках gnu-efi его не экспортируют в <efi.h> */
#ifndef HAVE_READY_TO_BOOT_GUID_EXTERN
...
STATIC EFI_STATUS GetBarReq(EFI_PCI_IO_PROTOCOL *P, UINT8 barIdx, BAR_REQ *out, UINT32 *origLo, UINT32 *origHi, BOOLEAN *is64)
{
...
STATIC VOID CollectUsedMmio(EFI_HANDLE *Handles, UINTN Count)
{
  ...
  for (UINTN i = 0; i < Count; i++)
  {
    ...
    if (EFI_ERROR(gBS->HandleProtocol(Handles[i], &gEfiPciIoProtocolGuid, (VOID **)&P)))
    {
      continue;
    }
...
STATIC VOID EFIAPI OnReadyToBoot(IN EFI_EVENT Event, IN VOID *Context)
{
  ...
  for (UINTN gi = 0; gi < gGpuCount; gi++)
  {
    ...
    EFI_STATUS prog = ProgramGpuBars(&gGpus[gi], chunkBase, pfPerGpu, &npCursor);
    if (EFI_ERROR(prog))
    {
