...
// ALLIGN FNC
STATIC UINT64
AlignUp64 (UINT64 x, UINT64 a)
{
  return (x + a - 1) & ~(a - 1);
}

STATIC UINT64
AlignDown64 (UINT64 x, UINT64 a)
{
  return x & ~(a - 1);
}

STATIC BOOLEAN
RangeOverlap (UINT64 a0, UINT64 a1, UINT64 b0, UINT64 b1)
{
  return !(a1 < b0 || b1 < a0);
}
...
// CFG READ FNC
STATIC UINT32
ReadCfg32 (EFI_PCI_IO_PROTOCOL *P, UINTN off)
{
  UINT32 v = 0;
...
STATIC UINT16
ReadCfg16 (EFI_PCI_IO_PROTOCOL *P, UINTN off)
{
  UINT16 v = 0;
...
STATIC UINT8
ReadCfg8 (EFI_PCI_IO_PROTOCOL *P, UINTN off)
{
  UINT8 v = 0;
...
// CFG WRITE FNC
STATIC VOID
WriteCfg32 (EFI_PCI_IO_PROTOCOL *P, UINTN off, UINT32 v)
{
  P->Pci.Write (P, EfiPciIoWidthUint32, off, 1, &v);
}

STATIC VOID
WriteCfg16 (EFI_PCI_IO_PROTOCOL *P, UINTN off, UINT16 v)
{
...
// MEM FNC
STATIC VOID
DisableMemDecode (EFI_PCI_IO_PROTOCOL *P, UINT16 *SaveCmd)
{
...
STATIC VOID
RestoreCmd (EFI_PCI_IO_PROTOCOL *P, UINT16 SaveCmd)
{
...
STATIC EFI_STATUS
GetBarReq (EFI_PCI_IO_PROTOCOL *P, UINT8 barIdx, BAR_REQ *out, UINT32 *origLo,
           UINT32 *origHi, BOOLEAN *is64)
{
  ...
  if (type == 0x2 && barIdx < 5)
    {
      ...
      UINT64 mask64
          = ((UINT64)maskHi << 32) | ((UINT64)maskLo & 0xFFFFFFF0ULL);
...
// GPU MMIO READDRESED
STATIC VOID
AddUsed (UINT64 base, UINT64 size)
{
...
STATIC VOID
CollectUsedMmio (EFI_HANDLE *Handles, UINTN Count)
{
  gUsedCount = 0;
...
STATIC BOOLEAN
FindFreeBlock (UINT64 size, UINT64 align, UINT64 *outBase)
{
  if (align < MAIN_ALIGN)
    {
      align = MAIN_ALIGN;
...
STATIC VOID
CollectBridges (EFI_HANDLE *Handles, UINTN Count)
{
  gBridgeCount = 0;
...
STATIC BRIDGE_CTX *
FindClosestBridgeForBus (UINT8 bus)
{
...
STATIC UINTN
BuildBridgeChain (UINT8 devBus, BRIDGE_CTX **outChain, UINTN maxChain)
{
  UINTN n      = 0;
  UINT8 curBus = devBus;
...
STATIC BOOLEAN
IsPciGpuOnly (GPU_CTX *G, BRIDGE_CTX **tmpChain, UINTN *tmpChainLen)
{
...
STATIC EFI_STATUS
CollectGpuDevices (EFI_HANDLE *Handles, UINTN Count)
{
  gGpuCount = 0;
...
STATIC EFI_STATUS
PlanMmio (UINT64 *pfBaseOut, UINT64 *pfPerGpuOut, UINT64 *npBaseOut,
          UINT64 *npNeedOut)
{
...
STATIC EFI_STATUS
WriteBarVerify (EFI_PCI_IO_PROTOCOL *P, UINT8 b, UINT64 base, BOOLEAN is64,
                UINT32 origLo)
{
  UINTN off = 0x10 + b * 4;
...
STATIC EFI_STATUS
ProgramGpuBars (GPU_CTX *G, UINT64 pfChunkBase, UINT64 pfChunkSize,
                UINT64 *npCursor)
{
  UINT16 savedCmd;
  DisableMemDecode (G->PciIo, &savedCmd);
  ...
  for (UINT8 b = 0; b < 6; b++)
    {
      ...
      EFI_STATUS st
          = WriteBarVerify (G->PciIo, b, base, G->Is64[b], G->OrigLo[b]);
...
STATIC VOID EFIAPI
OnReadyToBoot (IN EFI_EVENT Event, IN VOID *Context)
{
  EFI_STATUS Status;
